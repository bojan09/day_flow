...
export default function PomodoroReport({ pomodoroHistory }) {
  ...
  return (
    <Card>
      <p className="text-xs font-medium uppercase tracking-wider [color:var(--text-faint)] mb-4">⏱ Focus Sessions This Week</p>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        {[
          ...
          <div key={s.label} className="[background-color:var(--bg)] rounded-xl p-3 text-center">
            ...
            <p className="text-[10px] [color:var(--text-faint)] uppercase tracking-wide mt-0.5">{s.label}</p>
          </div>
        ))}
      </div>
        ...
        {report.days.map(d => {
          ...
          return (
            <div key={d.label} className="flex-1 flex flex-col items-center gap-1">
              {d.sessions > 0 && (
                <span className="text-[10px] [color:var(--text-faint)]">{d.sessions}</span>
              )}
              <div className="w-full [background-color:var(--bg-secondary)] rounded-t-md" style={{ height: '40px' }}>
                <div
