...
export default function AnalyticsPanel({ tasks }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <Card key={s.label} className="text-center !p-4">
            ...
            <p className="font-serif text-2xl text-forest-500">{s.val}</p>
      ...
      {/* Daily bar chart */}
      <Card>
        <p className="text-xs font-medium uppercase tracking-wider text-ink-faint mb-4">Daily Completions</p>
        <div className="flex items-end gap-2 h-20">
          {daily.map(d => {
            const pct = d.total > 0 ? (d.done / d.total) * 100 : 0
            return (
              <div key={d.label} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-[10px] text-ink-faint">{d.done}/{d.total}</span>
                <div className="w-full bg-stone-100 rounded-t-md" style={{ height: '48px' }}>
                  <div
                    className="w-full bg-forest-500 rounded-t-md transition-all duration-500"
      ...
      {categories.length > 0 && (
        <Card>
          <p className="text-xs font-medium uppercase tracking-wider text-ink-faint mb-3">By Category</p>
              ...
              return (
                <div key={cat}>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-xs text-ink-faint">{count} task{count > 1 ? 's' : ''}</span>
                  </div>
