    ...
    <div className="max-w-2xl mx-auto pt-2 space-y-4">
      <div className="relative">
        ...
        <input autoFocus value={query} onChange={e => setQuery(e.target.value)}
          ...
          className="w-full pl-11 pr-4 py-3.5 rounded-2xl border border-stone-200 bg-white text-ink text-sm outline-none focus:ring-2 focus:ring-forest-200 shadow-sm placeholder-ink-faint transition-all" />
        {query && (
          <button onClick={() => setQuery('')}
            className="absolute right-4 top-1/2 -translate-y-1/2 text-ink-faint hover:text-ink text-sm">✕</button>
        )}
      </div>

      {!q && <p className="text-center text-sm text-ink-faint italic pt-8">Start typing to search everything</p>}
      ...
      {results.length > 0 && (
        <div className="bg-white rounded-2xl border border-stone-100 shadow-sm divide-y divide-stone-50">
          ...
          {results.map((r, i) => (
            <div key={`${r.type}-${r.id}-${i}`} className="flex items-center gap-3 px-5 py-3.5 hover:bg-stone-50 transition-colors">
              <span className="text-xl flex-shrink-0">{r.icon}</span>
              <div className="flex-1 min-w-0">
                <p className="text-sm text-ink font-medium truncate">{highlight(r.title, query)}</p>
                {r.sub && <p className="text-xs text-ink-faint mt-0.5 truncate">{highlight(r.sub, query)}</p>}
              </div>
