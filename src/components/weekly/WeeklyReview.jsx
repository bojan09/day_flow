...
export default function WeeklyReview({ tasks, habits, mood }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-ink/30 backdrop-blur-sm px-4 pb-4 sm:pb-0">
      <div className="bg-white w-full sm:max-w-md rounded-3xl shadow-2xl overflow-hidden animate-scale-in">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={s.label} className="bg-parchment rounded-xl p-3 text-center">
                ...
                <p className="text-[10px] text-ink-faint uppercase tracking-wider mt-1">{s.label}</p>
              </div>
            ))}
          </div>
          {!saved ? (
            <div className="space-y-2">
              <p className="text-xs font-medium text-ink-muted uppercase tracking-wider">Next week I want to…</p>
              <textarea
                value={next}
                onChange={e => setNext(e.target.value)}
                placeholder="Set your intention for next week..."
                rows={2}
                className="w-full text-sm bg-parchment border border-stone-200 rounded-xl px-3 py-2 outline-none resize-none focus:ring-2 focus:ring-forest-200 text-ink placeholder-ink-faint/50"
              />
              <button onClick={() => setSaved(true)} className="w-full py-2.5 rounded-xl bg-forest-500 text-white text-sm font-medium hover:bg-forest-700 transition-colors">
                Save & Close
              </button>
              <button onClick={() => setDismissed(true)} className="w-full text-xs text-ink-faint hover:text-ink text-center transition-colors py-1">
                Skip for now
              </button>
            </div>
          ) : (
            <div className="text-center py-3 space-y-2">
              <p className="text-sm text-forest-500 font-medium">✓ Great week — on to the next one!</p>
              <button onClick={() => setDismissed(true)} className="text-xs text-ink-faint hover:text-ink transition-colors">Close</button>
            </div>
          )}
        </div>
      </div>
