...
const TYPE_STYLE = {
  ...
  note: 'bg-forest-50 border-forest-200 text-forest-700',
}
...
export default function BrainDump({ tasks, ideas, notes }) {
  ...
  if (step === 'done') return (
    <div className="max-w-lg mx-auto pt-8 text-center space-y-4">
      ...
      <h2 className="font-serif text-2xl text-ink">Brain dumped!</h2>
  ...
  return (
    <div className="max-w-2xl mx-auto pt-2 space-y-4">
        ...
        <>
          <div className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-stone-50 flex items-center justify-between">
              <p className="text-xs font-medium uppercase tracking-wider text-ink-faint">🧠 Brain Dump</p>
              <p className="text-xs text-ink-faint">{text.split('\n').filter(l => l.trim()).length} lines</p>
            </div>
            <textarea
              autoFocus
              value={text}
              ...
              className="w-full px-5 py-4 text-sm text-ink leading-relaxed bg-transparent outline-none resize-none min-h-72 placeholder-ink-faint/40 font-serif"
            />
          </div>
          <div className="flex items-center justify-between">
            <p className="text-xs text-ink-faint italic">Everything will be auto-sorted into tasks, ideas, or notes</p>
            <button onClick={handleParse} disabled={!text.trim()}
              className="px-5 py-2.5 rounded-full bg-forest-500 text-white text-sm font-medium hover:bg-forest-700 disabled:opacity-40 transition-colors">
              Sort it out →
            </button>
          </div>
        </>
      ...
      {step === 'review' && (
        <>
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium text-ink">Review & confirm — {parsed.filter(p => p.selected).length} selected</p>
            <button onClick={() => setStep('write')} className="text-xs text-ink-faint hover:text-ink transition-colors">← Back</button>
          </div>
          <div className="space-y-2">
            {parsed.map((item, i) => (
              <div key={i} className={`flex items-center gap-3 p-3 rounded-xl border ${item.selected ? TYPE_STYLE[item.type] : 'bg-stone-50 border-stone-100 opacity-50'}`}>
                <button onClick={() => toggleItem(i)}
                  className={`w-4 h-4 rounded border-2 flex-shrink-0 flex items-center justify-center text-[9px] transition-all ${
                    item.selected ? 'bg-white border-current text-current' : 'border-stone-300'
          ...
          <button onClick={handleSave} disabled={!parsed.some(p => p.selected)}
            className="w-full py-3 rounded-2xl bg-forest-500 text-white font-medium hover:bg-forest-700 disabled:opacity-40 transition-colors">
